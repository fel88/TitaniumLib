...
namespace Titanium.Web.Proxy
{
    ...
    public partial class ProxyServer : IDisposable
    {
        ...
        /// </summary>
        public event AsyncEventHandler<ITcpClient> OnClientConnectionCreate;

        /// <summary>
        ///     Customize TcpClient used for server connection upon create.
        /// </summary>
        public event AsyncEventHandler<ITcpClient> OnServerConnectionCreate;
            ...
            if (tcpClient != null)
            {
                Task.Run(async () =>
                {
                   ...
                   /* if (!FakeHttpStream.UseFake)
                    {
                        h = Dns.GetHostEntry(endPoint.IpAddress).HostName;
                        p = endPoint.Port;
                    }*/

                    var wrap = new TcpClientWrapper(tcpClient) { ClientToProxyClient = true, Host = h, Port = p };
                    await handleClient(wrap, endPoint);
                });
            }
        ...
        private async Task handleClient(ITcpClient tcpClient, ProxyEndPoint endPoint)
        {
        ...
        /// <returns></returns>
        internal async Task InvokeConnectionCreateEvent(ITcpClient client, bool isClientConnection)
        {
