...
namespace Titanium.Web.Proxy.Network.Tcp
{
    ...
    internal class TcpConnectionFactory : IDisposable
    {
        ...
        public readonly ConcurrentDictionary<string, ConcurrentQueue<TcpServerConnection>> cache
            ...
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                if (FakeHttpStream.UseFake)
                {
                    tcpClient = new TcpClientMy(new FakeHttpStream(isHttps) { Host = hostname, Port = port });
                }
                else
                {
                    tcpClient = new TcpClientWrapper(upStreamEndPoint)
                    {
                        Host = hostname,
                        Port = port,
                ...
                if (FakeHttpStream.UseFake)
                {
                    ipAddresses = new IPAddress[] { IPAddress.Parse("32.42.62.11") };
                }
                else
                {
                ...
                IStream prstrm = tcpClient.GetStream();

                stream = new CustomBufferedStream(prstrm, proxyServer.BufferPool, proxyServer.BufferSize);
                ...
                if (isHttps && !FakeHttpStream.UseFake)
                {

                    var sslStream = new SslStream(stream, false, proxyServer.ValidateServerCertificate, proxyServer.SelectClientCertificate);

                    stream = new CustomBufferedStream(new StreamWrapper(sslStream) { Host = hostname, Port = port, CacheWrapper = true }, proxyServer.BufferPool, proxyServer.BufferSize);
